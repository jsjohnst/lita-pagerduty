...
module Lita
  ...
  module Handlers
    ...
    class PagerdutyIncident < Handler
      ...
      route(
        ->(msg) { msg.body =~ /^#{config.cmd_prefix}\sincidents\sall$/ },
        :incidents_all,
        command: true,
        help: {
      ...
      route(
        ->(msg) { msg.body =~ /^#{config.cmd_prefix}\sincidents\smine$/ },
        :incidents_mine,
        command: true,
        help: {
      ...
      route(
        ->(msg) { msg.body =~ /^#{config.cmd_prefix}\sincident\s#{INCIDENT_ID_PATTERN}$/ },
        :incident,
        command: true,
        help: {
