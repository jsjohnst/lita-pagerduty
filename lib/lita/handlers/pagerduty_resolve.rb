...
module Lita
  ...
  module Handlers
    ...
    class PagerdutyResolve < Handler
      ...
      route(
        ->(msg) { msg.body =~ /^#{config.cmd_prefix}\sresolve\sall$/ },
        :resolve_all,
        command: true,
        help: {
      ...
      route(
        ->(msg) { msg.body =~ /^#{config.cmd_prefix}\sresolve\smine$/ },
        :resolve_mine,
        command: true,
        help: {
      ...
      route(
        ->(msg) { msg.body =~ /^#{config.cmd_prefix}\sresolve\s#{INCIDENT_ID_PATTERN}$/},
        :resolve,
        command: true,
        help: {
